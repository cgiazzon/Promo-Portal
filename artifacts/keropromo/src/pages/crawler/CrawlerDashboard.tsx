...
export default function CrawlerDashboard() {
  ...
  type SearchFilter = { term: string, limit: number, minPrice?: string, maxPrice?: string, state?: string, city?: string, sourceML?: boolean, sourceAmazon?: boolean };
  const [terms, setTerms] = useState<SearchFilter[]>([
    { term: "iphone 13 pro", limit: 15, minPrice: "2500", maxPrice: "4000", sourceML: true, sourceAmazon: false },
    { term: "fritadeira airfryer", limit: 30, maxPrice: "350", state: "SP", sourceML: true, sourceAmazon: false }
  ]);
  const [newTerm, setNewTerm] = useState("");
  const [newLimit, setNewLimit] = useState(20);
  const [newMin, setNewMin] = useState("");
  const [newMax, setNewMax] = useState("");
  const [newSourceML, setNewSourceML] = useState(true);
  const [newSourceAmazon, setNewSourceAmazon] = useState(false);
  ...
  const [capturedOffers, setCapturedOffers] = useState<{ id: string, term: string, price: string, link: string, thumbnail?: string, sourceLabel?: string }[]>([]);
  ...
  const handleAddTerm = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newSourceML && !newSourceAmazon) {
      alert("Selecione pelo menos uma plataforma fonte!");
      return;
    }
    if (newTerm.trim() && !terms.find(t => t.term === newTerm.trim())) {
      setTerms([...terms, {
        ...
        sourceML: newSourceML,
        sourceAmazon: newSourceAmazon
      }]);
      setNewTerm("");
      setNewMin("");
      setNewMax("");
  ...
  const handleEditTerm = (config: SearchFilter) => {
    ...
    setNewSourceML(config.sourceML ?? true);
    setNewSourceAmazon(config.sourceAmazon ?? false);
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      ...
      interval = setInterval(() => {
        if (terms.length === 0) {
          setLogs(prev => [{ time: new Date().toLocaleTimeString(), msg: "ERRO: Nenhum termo configurado para busca no Crawler.", type: "error" }, ...prev].slice(0, 50));
          setIsRunning(false);
          return;
        }
        ...
        // ------------------ MERCADO LIVRE (GHOST SCRAPER) ------------------
        if (config.sourceML !== false) {
          setLogs(prev => [
            { time: new Date().toLocaleTimeString(), msg: `[ML GHOST] Buscando ofertas secretas de: "${config.term}"...`, type: "info" },
            ...prev
          ].slice(0, 50));
          ...
          fetch(proxyURL)
            .then(res => {
              if (!res.ok) throw new Error("Firewall bloqueou túnel Proxy ML.");
              return res.json();
            })
            .then(data => {
              if (!data.contents) throw new Error("A vitrine interceptada no ML está vazia.");
              ...
              const results = Array.from(itemsNodeList)
                ...
                .map(el => {
                  ...
                  if (title && link) {
                    return {
                      id: "GHOST-ML-" + Math.random().toString(36).substr(2, 9),
              ...
              if (results.length > 0) {
                ...
                setLogs(prev => [
                  { time: new Date().toLocaleTimeString(), msg: `[ML GHOST] Sucesso ML! Extraídas ${results.length} un. válidas burlando a barreira.`, type: "success" },
                  ...prev
                ].slice(0, 50));
              } else {
                setLogs(prev => [
                  { time: new Date().toLocaleTimeString(), msg: `[ML GHOST] Mercado Livre sem itens para "${config.term}".`, type: "warning" },
                  ...prev
                ].slice(0, 50));
              }
            })
            .catch(err => {
               setLogs(prev => [{ time: new Date().toLocaleTimeString(), msg: `[ML ALARM] Barreira: ${err.message}`, type: "error" }, ...prev].slice(0, 50));
            });
        }

        // ------------------ AMAZON RAPID-API ------------------
        if (config.sourceAmazon) {
          setLogs(prev => [
            { time: new Date().toLocaleTimeString(), msg: `[AMAZON V2] Consumindo liminar RapidAPI Amazon para "${config.term}"...`, type: "warning" },
            ...prev
          ].slice(0, 50));

          const amazonUrl = `https://real-time-amazon-data.p.rapidapi.com/search?query=${encodeURIComponent(config.term)}&country=BR&limit=${config.limit || 15}`;

          fetch(amazonUrl, {
            headers: {
              'X-RapidAPI-Key': 'c193367ef0msh1175a5e30a6aaf6p126700jsn5c157e25944e',
              'X-RapidAPI-Host': 'real-time-amazon-data.p.rapidapi.com'
            }
          })
          .then(res => {
            if (res.status === 429) throw new Error("Cota de 100 Requisições Estourada na RapidAPI!");
            if (!res.ok) throw new Error(`Amazon API Erro Status: ${res.status}`);
            return res.json();
          })
          .then(data => {
            const items = data.data?.products || [];
            if (items.length > 0) {
              const amortizedOffers = items.slice(0, config.limit || 15).map((item: any) => ({
                id: "AMZ-" + item.asin,
                term: config.term,
                price: item.product_price ? item.product_price : "Indisponível",
              ...
              setCapturedOffers(prev => {
                const uniqueNew = amortizedOffers.filter((o: any) => !prev.find(p => p.id === o.id));
                return [...uniqueNew, ...prev].slice(0, 200);
              });

              setLogs(prev => [
                { time: new Date().toLocaleTimeString(), msg: `[AMAZON V2] Êxito Amazon! Foram rastreadas ${amortizedOffers.length} ofertas via RapidAPI.`, type: "success" },
                ...prev
              ].slice(0, 50));
            } else {
              setLogs(prev => [
                { time: new Date().toLocaleTimeString(), msg: `[AMAZON V2] Nenhum produto Amazon atendendo à pesquisa "${config.term}".`, type: "info" },
                ...prev
              ].slice(0, 50));
            }
          })
          .catch(err => {
             setLogs(prev => [{ time: new Date().toLocaleTimeString(), msg: `[AMAZON FATAL] API Quebrou: ${err.message}`, type: "error" }, ...prev].slice(0, 50));
          });
        }

        currentTermIndex = (currentTermIndex + 1) % terms.length;
      }, 12000); // 12 seconds de alívio para não comer as cotas Amazon 
    }
      ...
      <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 bg-white/5 p-2 rounded-2xl border border-white/10 backdrop-blur-md">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold transition-all duration-300 ${isRunning
        ...
        <nav className="flex flex-col gap-2">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-4 p-4 rounded-2xl text-left transition-all duration-300 border ${activeTab === item.id
              ...
              <div className="relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <span className="px-3 py-1 bg-green-500/20 text-green-400 border border-green-500/30 rounded-full text-xs font-bold flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> 2 Fontes Ativas
                  </span>
                </div>

                {/* Micro-Dashboard de Sources Conectadas */}
                <div className="flex flex-wrap items-center gap-3 mb-6 bg-white/5 border border-white/10 px-4 py-3 rounded-xl w-fit shadow-md">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mr-2">
                    <Activity className="w-4 h-4" /> Conexões Ativas:
                  </span>
                  <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 px-3 py-1.5 rounded-lg shadow-sm">
                    <Globe className="w-3.5 h-3.5 text-yellow-400" />
                    <span className="text-[10px] font-black text-yellow-200 uppercase tracking-wider">Mercado Livre</span>
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse shadow-[0_0_5px_#4ade80]"></span>
                  </div>
                  <div className="flex items-center gap-2 bg-orange-500/10 border border-orange-500/30 px-3 py-1.5 rounded-lg shadow-sm">
                    <Database className="w-3.5 h-3.5 text-orange-400" />
                    <span className="text-[10px] font-black text-orange-200 uppercase tracking-wider">Amazon (RapidAPI)</span>
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse shadow-[0_0_5px_#4ade80]"></span>
                  </div>
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="bg-gradient-to-b from-white/5 to-transparent border border-white/10 p-6 sm:p-8 rounded-3xl flex flex-col justify-start shadow-2xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                      {terms.map((config, i) => (
                        <div key={i} className="relative bg-[#090A0F]/80 backdrop-blur-sm border border-emerald-500/20 hover:border-emerald-500/50 p-4 rounded-xl flex flex-col group transition-all shadow-lg hover:shadow-[0_0_20px_rgba(52,211,153,0.1)]">
                          <div className="flex items-start justify-between gap-4 mb-2">
                            <div className="flex-1 overflow-hidden">
                              ...
                              <div className="flex gap-1.5 mb-2 mt-1">
                                 {config.sourceML !== false && <span className="text-[9px] bg-yellow-400/20 text-yellow-300 border border-yellow-400/30 px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">Mercado Livre</span>}
                                 {config.sourceAmazon && <span className="text-[9px] bg-orange-500/20 text-orange-300 border border-orange-500/30 px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">Amazon</span>}
                              </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-[11px] font-medium text-slate-400 mt-auto pt-2 border-t border-white/5">
                    ...
                    {!isFormExpanded ? (
                      ...
                      <form onSubmit={handleAddTerm} className="mt-auto bg-[#090A0F] border border-white/5 p-5 rounded-2xl flex flex-col gap-5 shadow-2xl border-t border-t-indigo-500/30">
                        {/* Seletor do HUB de Fontes */}
                        <div>
                          <label className="text-xs text-slate-400 font-bold mb-2 block uppercase tracking-widest">Ativar Fontes de Varredura para esse Termo</label>
                          <div className="bg-[#12141C] p-3 rounded-xl border border-white/5 flex flex-wrap gap-4 sm:gap-6 shadow-inner">
                             <label className="flex items-center gap-3 text-sm text-slate-300 font-bold cursor-pointer hover:text-white transition group">
                               <div className="relative flex items-center">
                                 <input type="checkbox" checked={newSourceML} onChange={e => setNewSourceML(e.target.checked)} className="peer sr-only" disabled={isRunning} />
                                 <div className="w-10 h-5 bg-white/10 peer-focus:outline-none rounded-full peer-checked:bg-yellow-500/50 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5 shadow-sm"></div>
                               </div>
                               🛒 Mercado Livre (+Ghost Proxy)
                             </label>
                             <label className="flex items-center gap-3 text-sm text-slate-300 font-bold cursor-pointer hover:text-white transition group">
                               <div className="relative flex items-center">
                                 <input type="checkbox" checked={newSourceAmazon} onChange={e => setNewSourceAmazon(e.target.checked)} className="peer sr-only" disabled={isRunning} />
                                 <div className="w-10 h-5 bg-white/10 peer-focus:outline-none rounded-full peer-checked:bg-orange-500/50 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5 shadow-sm"></div>
                               </div>
                               📦 Amazon (Limits: 100 queries)
                             </label>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <select
                              ...
                              disabled={isRunning}
                            >
                              <option value="" className="bg-slate-900">Todas do Brasil (Mercado Livre)</option>
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          ...
                          <button type="submit" disabled={isRunning || !newTerm.trim()} className="flex-[2] py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-bold transition shadow-[0_0_15px_rgba(79,70,229,0.5)] border border-indigo-500/50 disabled:opacity-50 disabled:shadow-none">Compilar Inteligência Restrita</button>
                        </div>
                      </form>
                    )}
                  </div>
                ...
                {/* Activity Feed */}
                <h3 className="text-sm font-bold tracking-widest text-slate-500 uppercase mb-4 flex items-center justify-between">
                   Live Terminal Logs
                   <span className="text-[10px] text-orange-500/80 bg-orange-500/10 px-2 py-0.5 rounded border border-orange-500/20">* Cota limitadora RapidAPI de 100 acessos/mês.</span>
                </h3>
                    ...
                    <ul className="space-y-2">
                      {logs.map((log, i) => (
                        <li key={i} className="flex items-start gap-4 p-2 hover:bg-white/5 rounded-lg transition-colors border border-transparent hover:border-white/5">
                          ...
                          <span className={`${log.type === 'info' ? 'text-blue-300' :
                        ...
                        {capturedOffers.map(oferta => (
                          <div key={oferta.id} className="px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl grid grid-cols-12 gap-4 items-center border border-transparent hover:border-white/10 transition-colors">
                            <div className="col-span-5 flex items-center gap-3">
                              <div className="w-10 h-10 rounded-md bg-white/10 flex items-center justify-center shrink-0 overflow-hidden relative">
                              ...
                              <div className="overflow-hidden">
                                ...
                                <div className="flex gap-2 items-center">
                                   {oferta.sourceLabel === 'AMZ' ? (
                                     <span className="text-[9px] font-black text-orange-400 tracking-wider">AMAZON</span>
                                   ) : (
                                     <span className="text-[9px] font-black text-yellow-400 tracking-wider">M.LIVRE</span>
                                   )}
                                   <div className="text-[10px] text-slate-500 font-mono">({oferta.id})</div>
                                </div>
                              </div>
                            </div>
